...
function initLoader() {
  ...
  const failsafeTimeout = setTimeout(() => {
    console.warn("Failsafe triggered: Loading took too long or crashed. Revealing portfolio.");
    try {
      if (!state.loaderComplete) {
        state.loaderComplete = true;
        
        // Hide loader overlay forcefully
        if (loaderOverlay) {
          loaderOverlay.style.opacity = 0;
          loaderOverlay.style.filter = "blur(20px)";
          setTimeout(() => {
